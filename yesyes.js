...
class YesCoinBot {
    ...
    async login(encodedData, proxy) {
        ...
        try {
            ...
            if (response.data.code === 0) {
                ...
                throw new Error(`Permintaan tidak berhasil: ${response.data.message}`);
            }
        } catch (error) {
            throw new Error(`Permintaan tidak berhasil: ${error.message}`);
        }
    }
    ...
    async makeRequest(method, url, data = null, token, proxy) {
        ...
        try {
            ...
            if (error.code === 'ECONNABORTED') {
                throw new Error(`request times out ${this.timeout}ms`);
            }
            throw new Error(`Permintaan tidak berhasil: ${error.message}`);
        }
    }
    ...
    async useSpecialBox(token, proxy) {
        ...
        try {
            ...
            if (response.code === 0) {
                await this.log('Aktifkan peti...', 'success');
                return true;
            } else {
                await this.log('Gagal mengaktifkan peti!', 'error');
                return false;
            }
        } catch (error) {
            return false;
        ...
        try {
            ...
            if (response.code === 0) {
                if (response.data.collectStatus) {
                    await this.log(`Buka peti, menerima ${response.data.collectAmount} Coins`, 'success');
    ...
    async attemptCollectSpecialBox(token, boxType, initialCoinCount, proxy) {
        ...
        await this.log('Tidak bisa mengumpulkan peti!', 'error');
        return 0;
    }
    ...
    async recoverCoinPool(token, proxy) {
        ...
        try {
            const response = await this.makeRequest('post', url, {}, token, proxy);
            if (response.code === 0) {
                await this.log('Pemulihan berhasil!', 'success');
                return true;
            } else {
    ...
    async getTaskList(token, proxy) {
        ...
        try {
            ...
            if (response.code === 0) {
                ...
                await this.log(`Tidak dapat memperoleh daftar tugas: ${response.message}`, 'error');
                return null;
            }
        } catch (error) {
    ...
    async finishTask(token, taskId, proxy) {
        ...
        try {
            ...
            if (response.code === 0) {
                await this.log(`Kerjakan tugasnya${taskId} sukses | menghadiahkan: ${response.data.bonusAmount}`, 'success');
                return true;
            } else {
                await this.log(`Kerjakan tugasnya${taskId} gagal: ${response.message}`, 'error');
                return false;
            }
        } catch (error) {
            await this.log(`Kesalahan saat melakukan misi: ${error.message}`, 'error');
            return false;
        }
    }
    ...
    async upgradeLevel(token, currentLevel, targetLevel, upgradeType, proxy) {
        ...
        while (currentLevel < targetLevel) {
            try {
                ...
                if (response.code === 0) {
                    currentLevel++;
                    await this.log(`Meningkatkan ${upgradeTypeName} naik Lv ${currentLevel}`, 'success');
                } else {
                    await this.log(`Peningkatan gagal: ${response.message}`, 'error');
                    break;
                }
            } catch (error) {
                await this.log('Kesalahan peningkatan: ' + error.message, 'error');
                break;
            }
        }

        if (currentLevel === targetLevel) {
            await this.log(`${upgradeTypeName} sudah berada pada level tersebut ${currentLevel}`, 'info');
        }
    }
            ...
            if (accountBuildInfo.code === 0) {
                ...
                if (swipeBotLevel < 1) {
                    ...
                    if (upgradeResponse.code === 0) {
                        ...
                        await this.log('Pembelian SwipeBot gagal', 'error');
                    }
                }
                ...
                if (swipeBotLevel >= 1 && !openSwipeBot) {
                    ...
                    if (toggleResponse.code === 0) {
                        await this.log('Aktifkan SwipeBot ', 'success');
                    } else {
                ...
                if (swipeBotLevel >= 1 && openSwipeBot) {
                    ...
                    if (offlineBonusInfo.code === 0 && offlineBonusInfo.data.length > 0) {
                        ...
                        if (claimResponse.code === 0) {
                            await this.log(`Klaim bonus offline berhasil, menerima${claimResponse.data.collectAmount} coins`, 'success');
                        } else {
                            await this.log('Klaim bonus offline gagal', 'error');
                        }
                    }
                }
            } else {
                await this.log('Tidak dapat memperoleh informasi SwipeBot', 'error');
            }
        } catch (error) {
            await this.log(`Kesalahan saat menangani SwipeBot: ${error.message}`, 'error');
        }
    }
    ...
    async main() {
        try {
            try {
                ...
                await this.log(`Proxy IP: Unknown: ${error.message}`, 'error');
                return;
            }
            ...
            try {
                ...
                await this.log(`Tidak dapat mengambil token: ${error.message}`, 'error');
                return;
            }

            await this.performTasks();
        } catch (error) {
            await this.log(`Ada kesalahan: ${error.message}`, 'error');
        } finally {
            if (!isMainThread) {
        ...
        try {
            ...
            if (response.code === 0) {
                ...
                if (bonusInfo.commonTaskBonusStatus === 1) {
                    ...
                    if (claimResponse.code === 0) {
                        await this.log(`Berhasil klaim bonus Tugas | menghadiahkan ${claimResponse.data.bonusAmount}`, 'success');
                        return true;
                    } else {
                        await this.log(`Klaim bonus Tugas gagal: ${claimResponse.message}`, 'error');
                        return false;
                    }
                } else {
                    await this.log('Tidak memenuhi syarat untuk menerima bonus Tugas', 'info');
                    return false;
                }
            } else {
                await this.log(`Tidak dapat memperoleh bonus: ${response.message}`, 'error');
                return false;
            }
        } catch (error) {
            await this.log(`Kesalahan saat memeriksa dan mengklaim bonus Tugas: ${error.message}`, 'error');
            return false;
        }
    }
    ...
    async performTasks() {
        try {
            ...
            await this.log(`Akun: ${nickname}`, 'info');
            ...
            if (squadInfo && squadInfo.data.isJoinSquad) {
                ...
                await this.log(`Squad: ${squadTitle} | ${squadMembers} Anggota`, 'info');
            } else {
                await this.log('Squad: Anda tidak berada dalam Squad, bergabunglah dengan Dan Cay Airdrop.', 'warning');
                ...
                if (joinResult) {
                    await this.log(`Squad: ${nickname} gia nhập Squad sukses !`, 'success');
                } else {
                    await this.log(`Squad: ${nickname} gia nhập Squad gagal !`, 'error');
                }
            }
            ...
            if (balance === null) {
                await this.log('Saldo: Tidak dapat membaca saldo', 'error');
            } else {
            ...
            if (gameInfo === null) {
                await this.log('banned lo, mampos wkwkwk!', 'error');
            } else {
            ...
            if (collectInfo === null) {
                ...
                await this.log(`Energi yang tersisa ${coinPoolLeftCount}`, 'info');
                ...
                if (coinPoolLeftCount > 0) {
                    ...
                    if (collectResult && collectResult.code === 0) {
                        ...
                        await this.log(`berhasil, terima ${collectedAmount} coins`, 'success');
                    } else {
                        await this.log('Tap tidak berhasi!', 'error');
                    }
                }
            }
                ...
                if (recoverResult) {
                    ...
                    if (updatedCollectInfo) {
                        ...
                        if (coinPoolLeftCount > 0) {
                            ...
                            if (collectResult && collectResult.code === 0) {
                                const collectedAmount = collectResult.data.collectAmount;
                                await this.log(`recovery berhasil, terima ${collectedAmount} coins`, 'success');
                            } else {
                                await this.log('recovery tidak berhasil!', 'error');
                            }
                        }
                    }
                }
...
if (isMainThread) {
    ...
    async function processCycle() {
        console.log('kalo takut, jangan digunain LEMAH!...'.magenta);
        let accountQueue = [...accounts];

        function startWorker() {
            if (accountQueue.length === 0) {
                if (activeWorkers === 0) {
                    console.log('kelar semua, ngaso dulu.'.green);
            ...
            worker.on('exit', (code) => {
                if (code !== 0) {
                    console.error(`awkwkwk error ler, berhenti dengan kode ${code}`.red);
                }
                activeWorkers--;
                startWorker();
            });
